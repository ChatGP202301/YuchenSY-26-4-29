...
if (success) {
success.innerHTML = form.dataset.successMessage || inquirySuccessHtml;
success.hidden = false;
success.setAttribute('role', 'status');
success.setAttribute('tabindex', '-1');
success.focus({ preventScroll: true });
}
if (accessKey && window.sessionStorage) {
...
if (!modKey || isEditable(e.target)) return;
if (key === 'a' || key === 'c' || key === 'x' || key === 'p' || key === 's' || key === 'u') {
e.preventDefault();
}
}, true);
