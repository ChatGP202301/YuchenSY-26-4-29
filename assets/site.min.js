...
(function(){
  ...
  function validateContactChoice(form){
    ...
    if (email) {
      email.setCustomValidity('Enter a work email or a WhatsApp number so we can reply.');
...
(function addTechnicalArticlesLink(){
  ...
  document.querySelectorAll('.nav-resources-menu').forEach(function(menu){
    // The OEM Products menu has an intentionally fixed four-entry taxonomy.
    // Technical Articles belongs only to an actual Resources dropdown.
    if (menu.closest('[data-oem-products-menu="true"]')) return;
    if (menu.querySelector('[data-technical-articles-link]')) return;
    var link = document.createElement('a');
    link.href = /\/en\/resources\//.test(location.pathname) ? './' : 'resources/';
    link.textContent = 'Technical Articles';
...
(function loadYuchenMeasurement(){
  if (new URLSearchParams(window.location.search).get('yw_download_embed') === '1') return;
  ...
  configScript.src = new URL('measurement-config.js?v=20260810', base).href;
  configScript.defer = true;
  configScript.dataset.yuchenMeasurement = 'config';
  configScript.onload = function(){
    var measurementScript = document.createElement('script');
    measurementScript.src = new URL('measurement.js?v=20260810', base).href;
...
// Load one shared, localized download surface while preserving every real fallback URL.
(function loadYuchenDownloadCenter(){
  if (document.querySelector('script[data-yuchen-download-center]')) return;
  var ownScript = document.currentScript || Array.prototype.find.call(document.scripts, function(script){ return /\/assets\/site\.min\.js(?:\?|$)/.test(script.src || ''); });
  if (!ownScript || !ownScript.src) return;
  var base = new URL('.', ownScript.src);
  if (!document.querySelector('link[data-yuchen-download-center]')) {
    var stylesheet = document.createElement('link');
    stylesheet.rel = 'stylesheet';
    stylesheet.href = new URL('download-center.css?v=20260814', base).href;
    stylesheet.dataset.yuchenDownloadCenter = 'style';
    document.head.appendChild(stylesheet);
  }
  var runtime = document.createElement('script');
  runtime.src = new URL('download-center.js?v=20260814', base).href;
  runtime.defer = true;
  runtime.dataset.yuchenDownloadCenter = 'runtime';
