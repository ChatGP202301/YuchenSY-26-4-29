#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Add 20-inch frame RO water machine 400G-1200G across language pages."""
...
NEW_SLUG = "product-20-inch-frame-ro-water-machine-400g-1200g.html"
PRODUCT_ID = "20-inch-frame-ro-water-machine-400g-1200g"
MAIN_IMAGE = "20-inch-frame-ro-water-machine-400g-1200g-oem.webp"
IMAGE_WIDTH = 374
IMAGE_HEIGHT = 582
AFTER_SLUG = "product-front-panel-10-inch-frame-ro-water-machine-400g-800g.html"
TODAY = "2026-06-26"
BRAND = "Yuchen Water"
...
TWENTY_INCH = {
    ...
    "uz": "20 dyuymli",
    "vi": "20 inch",
    "zu": "ama-intshi angu-20",
}


PRODUCT_NAME_OVERRIDES = {
    "bs": "20-inčna okvirna RO mašina za vodu",
    "en": "20-Inch Frame RO Water Machine",
    "pl": "20-calowa ramowa maszyna do wody RO",
    "ru": "RO-машина для воды на 20-дюймовой раме",
    "sr": "20-инчна рамска RO машина за воду",
    "sr-me": "20-inčna ramovska RO mašina za vodu",
    "uk": "RO-машина для води на 20-дюймовій рамі",
}
...
def product_name(lang: str) -> str:
    ...
    return PRODUCT_NAMES[lang].replace(TEN_INCH[lang], TWENTY_INCH[lang], 1)


def treatment_value(lang: str) -> str:
    twenty = TWENTY_INCH[lang]
    post = POST_CARBON[lang]
    return f"{twenty} PP + {twenty} UDF + {twenty} PP + RO + T33 {post}"
...
def update_products_page(lang: str) -> None:
    ...
    if re.search(pattern, text, flags=re.S):
        ...
        for anchor in [
            AFTER_SLUG,
            "product-10-inch-frame-ro-water-machine-400g-800g.html",
...
def update_products_json() -> None:
    ...
    entry = {
        ...
        "summary": copy["intro"],
        "features": [
            "400G-1200G 20-inch frame RO water machine for municipal tap water",
            "20-inch PP, 20-inch UDF, 20-inch PP, RO membrane and T33 post-carbon stages",
            "Microcomputer control with full automatic intelligent operation",
            "Frame cabinet structure for commercial drinking water and distributor projects",
            "OEM/ODM color, logo, label, packaging and configuration support",
        ],
        "applications": "Municipal tap-water purification for offices, cafés, retail stores, schools, distributors and OEM private-label RO machine projects.",
        "related": [
            "front-panel-10-inch-frame-ro-water-machine-400g-800g",
    ...
    for index, item in enumerate(products):
        ...
        insert_at = next(
            (
                ...
                if item.get("id") == "front-panel-10-inch-frame-ro-water-machine-400g-800g"
            ),
            10,
        )
...
def update_ai_files() -> None:
    line = f"- 20-Inch Frame RO Water Machine 400G-1200G: https://www.yuchensy.com/en/{NEW_SLUG}\n"
